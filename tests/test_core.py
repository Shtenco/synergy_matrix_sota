...
def test_callable_principal_is_not_term_credit_funding() -> None:
    book = MaturityBook(
        [
            tranche("callable", "u1", "100", 0, 40, liquidity_class="CALLABLE"),
            tranche("term", "u2", "250", 0, 40),
        ]
    )
    assert book.immediately_callable_principal(0) == Decimal("100")
    assert book.eligible_funding(as_of_week=0, loan_maturity_weeks=12) == Decimal("250")


def test_mixed_currency_totals_require_explicit_denomination() -> None:
    book = MaturityBook(
        [
            tranche("usd", "u1", "100", 0, 20, denomination="USDT"),
            tranche("kzt", "u2", "50000", 0, 20, denomination="KZT"),
        ]
    )
    with pytest.raises(MaturityInvariantError):
        book.total_open_principal(0)
    assert book.total_open_principal(0, "USDT") == Decimal("100")
    assert book.total_open_principal(0, "KZT") == Decimal("50000")
